...
QSqlQueryModel* Livraison::rechercheParref_l(const QString& ref_l) {
    QSqlQueryModel* model = new QSqlQueryModel();
    QSqlQuery query;
    query.prepare("SELECT * FROM livraison WHERE ref_l LIKE :ref_l");
    query.bindValue(":ref_l", "%" + ref_l + "%"); // Search for partial matches
